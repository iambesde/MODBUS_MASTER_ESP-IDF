...
#include "mb.h"
#include "mbport.h"
static const char *TAG = "MODBUS_MB_C:";
#define MB_LOG(...) 
        ...
        if( ret > 0 )
        {
            if( FD_ISSET( xClientSocket, &fread ) )
            {
                ...
                if( usTCPBufPos >= MB_TCP_FUNC )
                {
                    ...
                    {
                        ( void )xMBMasterPortEventPost( EV_FRAME_RECEIVED );
                        return TRUE;
                    }
